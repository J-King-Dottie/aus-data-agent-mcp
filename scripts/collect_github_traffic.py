...
def _fetch_json(repo: str, metric: str, token: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            f"GitHub traffic request failed for {metric}: HTTP {exc.code}: {body}"
        ) from exc
...
def _merge_metric(data_dir: Path, metric: str, payload: dict[str, Any], collected_at: str) -> None:
    ...
    if not isinstance(series, list):
        raise RuntimeError(
            f"GitHub traffic payload for {metric} does not contain a '{metric}' list."
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--repo", default=os.getenv("GITHUB_REPOSITORY", ""), help="Repository in owner/name form."
    )
    parser.add_argument(
        "--data-dir", default="github-traffic", help="Directory for traffic CSV files."
    )
    ...
    if not token:
        print(
            "Missing token. Set GH_TOKEN or GITHUB_TOKEN with repository traffic access.",
            file=sys.stderr,
        )
        return 2
