...
from . import data_config  # Load optional .env before resolving paths.

RUNTIME_DIR = Path(os.getenv("AUSDATA_RUNTIME_DIR") or Path(__file__).resolve().parents[1] / "runtime").resolve()
...
if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]{0,127}", SESSION_ID):
    raise ValueError("AUSDATA_SESSION_ID must be 1-128 letters, digits, dots, underscores or hyphens, starting with a letter or digit.")
