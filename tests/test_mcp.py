...
class CatalogueTests(unittest.TestCase):
    def setUp(self):
        ...
        entries = [{"datasetId": f"test::{i}", "provider": provider, "title": title,
                    "description": "", "searchText": title, "sourceUrl": "https://example.test",
                    "requiresMetadataBeforeRetrieval": False}
                   for i, (provider, title) in enumerate([("ABS", "Zebra labour wages"), ("ABS", "Alpha labour"), ("World Bank", "labour wages")])]
        self.snapshot.write_text(json.dumps({"schema_version": catalog.SCHEMA_VERSION, "generation": "fixture", "entries": entries,
                                            "lastUpdated": "fixture", "sources": {provider: {"status": "fresh", "error": None, "refresh_after": time.time() + 3600}
    ...
    def test_candidate_pool_paginates_in_fts_order(self):
        ...
        second = catalog.search_unified_catalog("labour wages", limit=1, offset=first["next_offset"], provider="ABS")
        self.assertEqual([first["candidates"][0]["title"], second["candidates"][0]["title"]],
...
class ProviderTests(unittest.TestCase):
    def test_comtrade_requires_explicit_scope_before_network_request(self):
        entry = macro_data.MacroCatalogEntry(entry_id="comtrade::goods_trade", provider_key="comtrade",
                                            provider_name="UN Comtrade", concept_id="goods_trade",
                                            concept_label="Goods trade", indicator_label="Goods trade",
                                            unit="USD", description="", search_text="", provider_config={})
        args = dict(reporter_codes=["36"], partner_codes=["0"], flow_code="", frequency_code="A",
                    hs_codes=["TOTAL"], start_year=2020, end_year=2021)
        with patch.object(macro_data.httpx, "get", side_effect=AssertionError("No provider request expected")) as get:
            with self.assertRaisesRegex(ValueError, "flowCode"):
                macro_data._fetch_comtrade("Australian exports", entry, {}, **args)
            args["flow_code"] = "X"
            args["hs_codes"] = []
    ...
    def test_comtrade_rejects_mixed_valid_and_invalid_codes(self):
        entry = macro_data.MacroCatalogEntry(entry_id="comtrade::goods_trade", provider_key="comtrade",
                provider_name="UN Comtrade", concept_id="goods_trade", concept_label="Goods trade",
                indicator_label="Goods trade", unit="USD", description="", search_text="", provider_config={})
        args = dict(reporter_codes=["36"], partner_codes=["0"], flow_code="X", frequency_code="A",
                    hs_codes=["TOTAL"], start_year=2022, end_year=2022)
        with patch.object(macro_data.httpx, "get", side_effect=AssertionError("No provider request expected")) as get:
            for field, invalid in (("reporter_codes", "999999"), ("partner_codes", "999999"),
                                   ("hs_codes", "999999")):
                bad = {**args, field: [*args[field], invalid]}
                with self.subTest(field=field), self.assertRaisesRegex(ValueError, "Choose codes from get_metadata"):
                    macro_data._fetch_comtrade("Australian exports", entry, {}, **bad)
            get.assert_not_called()
    ...
    def test_comtrade_metadata_covers_frequency_and_retrieval_keeps_scope(self):
        ...
        row = {
            "period": 2022, "primaryValue": 100, "reporterCode": 36, "partnerCode": 0,
            "cmdCode": "TOTAL", "reporterDesc": "Australia", "partnerDesc": "World",
        }
        response = httpx.Response(200, request=httpx.Request("GET", "https://example.test/trade"), json={"data": [row, row]})
        with patch.object(macro_data.httpx, "get", return_value=response) as get:
            result = macro_data._fetch_comtrade("Australian exports", entry, {}, reporter_codes=["36"],
                    partner_codes=["0"], flow_code="X", frequency_code="A", hs_codes=["TOTAL"],
                    start_year=2022, end_year=2022)
        self.assertEqual(result["series"][0]["points"], [{"x": "2022", "y": 100.0}])
        ...
        with patch.object(macro_data.settings, "comtrade_api_key", "dummy-test-key"):
            with patch.object(macro_data.httpx, "get", return_value=response) as authenticated_get:
                authenticated = macro_data._fetch_comtrade("Australian exports", entry, {}, reporter_codes=["36"],
                        partner_codes=["0"], flow_code="X", frequency_code="A", hs_codes=["TOTAL"],
                        start_year=2022, end_year=2022)
        self.assertEqual(authenticated_get.call_args.kwargs["headers"]["Ocp-Apim-Subscription-Key"], "dummy-test-key")
        ...
        conflicting = httpx.Response(200, request=httpx.Request("GET", "https://example.test/trade"),
        ...
        with patch.object(macro_data.httpx, "get", return_value=conflicting):
            with self.assertRaisesRegex(RuntimeError, "conflicting values"):
                macro_data._fetch_comtrade("Australian exports", entry, {}, reporter_codes=["36"],
                        partner_codes=["0"], flow_code="X", frequency_code="A", hs_codes=["TOTAL"],
                        start_year=2022, end_year=2022)

    def test_oecd_rejects_ambiguous_dimensions(self):
        entry = macro_data.MacroCatalogEntry(entry_id="oecd::TEST", provider_key="oecd",
                provider_name="OECD", concept_id="TEST", concept_label="Test", indicator_label="Test",
                unit="", description="", search_text="", provider_config={})
        csv_text = "REF_AREA,TIME_PERIOD,OBS_VALUE,MEASURE\nAUS,2022,1,A\nAUS,2022,2,B\n"
        response = httpx.Response(200, request=httpx.Request("GET", "https://example.test/oecd"), text=csv_text)
        config = {"agency": "OECD.TEST", "dataflow": "TEST", "version": "1.0"}
        with patch.object(macro_data.httpx, "get", return_value=response):
            with self.assertRaisesRegex(RuntimeError, "multiple observations for the same country and period"):
                macro_data._fetch_oecd("Test", entry, config, ["AUS"], 2022, 2022)

    def test_oecd_reads_unambiguous_series(self):
        entry = macro_data.MacroCatalogEntry(entry_id="oecd::TEST", provider_key="oecd",
                provider_name="OECD", concept_id="TEST", concept_label="Test", indicator_label="Test",
                unit="Index", description="", search_text="", provider_config={})
        ...
        config = {"agency": "OECD.TEST", "dataflow": "TEST", "version": "1.0"}
        with patch.object(macro_data.httpx, "get", return_value=response):
            result = macro_data._fetch_oecd("Test", entry, config, ["AUS"], 2021, 2022)
        self.assertEqual(result["series"][0]["points"], [{"x": "2021", "y": 1.0}, {"x": "2022", "y": 2.0}])
    ...
    def test_world_bank_requests_countries_and_all_pages(self):
        entry = macro_data.MacroCatalogEntry(entry_id="test", provider_key="worldbank", provider_name="World Bank",
                                            concept_id="test", concept_label="Test", indicator_label="Test", unit="USD", provider_config={}, description="", search_text="")
        def response(year, page):
            return httpx.Response(200, request=httpx.Request("GET", f"https://example.test?page={page}"), json=[
                {"pages": 2}, [{"countryiso3code": "AUS", "date": year, "value": 2}]
            ])
        with patch.object(macro_data.httpx, "get", side_effect=[response("2021", 1), response("2020", 2)]) as get:
            result = macro_data._fetch_world_bank("", entry, {"series_id": "TEST", "source_id": "2"}, ["AUS"], 2020, 2021)
