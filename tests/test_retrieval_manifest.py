...
from ausdata_mcp import artifacts
...
class RetrievalManifestTests(unittest.TestCase):
    def setUp(self):
        ...
        patcher = patch.object(artifacts, "RUNTIME_DIR", self.root)
        patcher.start()
    ...
    def test_large_artifact_manifest_prompts_size_notice(self):
        payload = {
            "series": [
                {"seriesKey": "test", "observations": [{"observationKey": "2024", "value": 1}]}
            ]
        }
        with patch.object(artifacts, "LARGE_ARTIFACT_BYTES", 1):
            manifest = artifacts.store_retrieval(payload, "ABS,TEST,1.0", "Test")
        self.assertTrue(manifest["large_artifact"])
        self.assertIn("large local file", manifest["size_notice"])
        self.assertEqual(
            manifest["artifact_size_mib"], round(manifest["artifact_bytes"] / 1048576, 2)
        )
    ...
    def test_domestic_manifest_preserves_raw_dimensions_and_attributes(self):
        ...
        manifest = artifacts.store_retrieval(payload, "pdh::TEST", "Population")
        saved = json.loads(Path(manifest["artifact_path"]).read_text(encoding="utf-8"))
    ...
    def test_abs_manifest_uses_time_dimension_instead_of_observation_index(self):
        ...
        manifest = artifacts.store_retrieval(payload, "ABS,LF_AGES,1.0.0", "Labour Force")
