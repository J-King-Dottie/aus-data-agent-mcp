...
- Search with concise subject/source terms. Results are text matches of catalogue records, not suitability rankings or an index of every dimension value a publisher releases. If a measure term yields poor matches, search for the relevant collection and inspect its dimensions. Select by metric, geography, population, frequency and actual period coverage; page or refine searches when needed. A fresh catalogue does not establish current observations. Inspect source freshness and unavailable-source warnings.
- Browse codes using the dataset's dimension IDs and follow `next_offset`. Code previews are incomplete. Copy source codes from metadata verbatim; labels and normalized search text are not retrieval codes. If several codes match, select by their definitions; do not guess nearby codes or substitute categories silently. Choose narrow or broad retrieval as useful, respecting provider rate limits. Independent datasets can be retrieved concurrently; keep dependent calls ordered.
- Reuse codes already shown in metadata; browse only codes you still need. Codelists validate individual codes, not whether their combination has observations. When unsure which combination exists, start with the filters you know, inspect the returned series dimensions, then narrow. A dataflow may already identify the measure.
...
- Stay within the MCP's supported pathways for data acquisition. If no suitable dataset is retrievable from a source feed, name the feed checked and the specific gap, such as a missing dataflow or dimension. Other publisher channels may have the data; do not describe the gap as a general MCP limit or claim the publisher lacks the data. Ask before using websites, other APIs, workbooks or manual transcription unless already authorized.
