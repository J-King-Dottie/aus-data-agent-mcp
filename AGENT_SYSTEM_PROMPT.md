...
- Reuse codes already shown in metadata; browse only codes you still need. Codelists do not prove a combination has data. When a dataflow already identifies the measure, filter the requested area and period and inspect remaining dimensions in the evidence; add other filters when the question requires them.
